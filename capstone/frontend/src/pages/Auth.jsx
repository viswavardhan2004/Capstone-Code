...
export default function Auth() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Handle OTP for LPU students
    if (!isLogin && formData.email.toLowerCase().endsWith('@lpu.in') && !showOTP) {
      try {
        ...
        if (response.ok) {
          setShowOTP(true);
          setAlert({ message: '📩 OTP sent to your LPU email!', type: 'success' });
        } else {
          setAlert({ message: `❌ ${data.error || 'Failed to send OTP'}`, type: 'error' });
        }
      } catch (error) {
        console.error('OTP error:', error);
        setAlert({ message: '⚠️ Error sending OTP', type: 'error' });
      }
      return;
    }
    ...
    if (formData.email === 'karanvir@gmail.com' && formData.password === '1234') {
      ...
      setAlert({ message: '🔐 Admin Login Successful! Redirecting to Admin Panel...', type: 'success' });
      setTimeout(() => { window.location.href = 'http://localhost:5174/admin/fraud-alerts'; }, 1500);
      return;
    }
    ...
    try {
      ...
      const payload = isLogin 
        ...
        : { email: formData.email, password: formData.password, name: formData.name, role: userType, otp };
      ...
      <div className="max-w-md w-full relative z-10">
        ...
        <div className="rounded-2xl p-8 border shadow-xl backdrop-blur-lg hover:shadow-2xl hover:scale-[1.02] transition-all duration-300" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-color)' }}>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {!isLogin && showOTP && (
              <div>
                <label className="block text-sm font-semibold mb-2" style={{ color: 'var(--text-primary)' }}>Enter OTP</label>
                <input 
                  type="text" 
                  value={otp}
                ...
                <p className="text-xs mt-2 text-blue-400">
                  Please check your @lpu.in email for the verification code.
                </p>
              </div>
            )}
            ...
            >
              {isLogin ? 'Sign In' : (showOTP ? 'Verify & Register' : 'Create Account')}
            </button>
          </form>
